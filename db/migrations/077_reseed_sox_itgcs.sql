-- Migration 077: Re-seed SOX ITGCs that failed in 013 due to invalid 'it-dependent' enum value.
-- sox.control_type only accepts ('manual','automated','itdm'); 'it-dependent' caused the entire
-- 013 INSERT to fail, leaving sox.itgcs empty. This migration inserts all 32 ITGCs idempotently.
...
(uuid_generate_v4(), 't_maybank', 'MAYX-ITGC-002', 'Joiners/movers/leavers reviewed monthly',   'HR access lifecycle events reviewed against AD/IdP within 30 days.',     'Ensure access rights reflect current employment status.',               'itdm',      'monthly',    'effective',   now() - interval '20 days'),
...
(uuid_generate_v4(), 't_maybank', 'MAYX-ITGC-015', 'Patch cadence < 30 days',                   'Critical and high CVEs patched within 30 days of disclosure.',            'Reduce attack surface from known vulnerabilities.',                     'itdm',      'monthly',    'effective',   now() - interval '8 days'),
...
(uuid_generate_v4(), 't_maybank', 'MAYX-ITGC-018', 'Disk encryption at rest enforced',          'All production storage volumes encrypted with AES-256.',                 'Protect data at rest from physical or logical unauthorised access.',    'automated', 'continuous', 'effective',   now() - interval '1 days'),
...
(uuid_generate_v4(), 't_grab',    'GRAB-ITGC-002', 'Joiners/movers/leavers reviewed monthly',   'Access lifecycle events reviewed monthly.',                               'Ensure access reflects employment status.',                             'itdm',      'monthly',    'effective',   now() - interval '28 days'),
...
(uuid_generate_v4(), 't_grab',    'GRAB-ITGC-008', 'Patch cadence < 30 days',                   'Critical CVEs patched within 30 days.',                                   'Reduce known vulnerability exposure.',                                  'itdm',      'monthly',    'effective',   now() - interval '10 days'),
...
-- Re-seed KCAs for any newly inserted ITGCs
...
-- Re-seed walkthroughs for any newly inserted ITGCs
...
-- Re-seed deficiencies for any newly inserted deficiency-status ITGCs
