<script lang="ts">
  ...
  import {
    Download, Printer, Bot, FileBarChart, AlertTriangle, ShieldCheck,
...
<div class="mx-auto max-w-5xl px-2 sm:px-4">
  ...
  <section class="mt-10 border-t border-slate-200 pt-8">
    ...
    <div class="space-y-4 text-[15px] leading-7 text-slate-700">
      {#each paragraphs as p, i}
        <p class="{i === 0 ? 'first-letter:float-left first-letter:mr-2 first-letter:mt-0.5 first-letter:font-mono first-letter:text-5xl first-letter:font-semibold first-letter:leading-none first-letter:text-slate-900' : ''}">
          {p}
        </p>
      {/each}
    </div>
  ...
  <section class="mt-10 border-t border-slate-200 pt-8">
    <header class="mb-6 flex items-baseline justify-between">
      <div class="flex items-baseline gap-4">
        <span class="font-mono text-sm tabular-nums text-slate-400">02.</span>
        <h3 class="font-serif text-2xl font-bold text-grc-ink">Top 5 Risks (Residual)</h3>
      </div>
      <span class="text-[11px] text-slate-400">Quantified by Risk Quantifier · FAIR</span>
    </header>
  ...
  <section class="mt-10 border-t border-slate-200 pt-8">
    <header class="mb-6 flex items-baseline justify-between">
      <div class="flex items-baseline gap-4">
        <span class="font-mono text-sm tabular-nums text-slate-400">04.</span>
        <h3 class="font-serif text-2xl font-bold text-grc-ink">Third-Party Concentration</h3>
      </div>
      <span class="text-[11px] text-slate-400">{data.concentrations.length} concentrations · {data.vendors.length} vendors</span>
    </header>
  ...
  <section class="mt-10 border-t border-slate-200 pt-8">
    <header class="mb-6 flex items-baseline justify-between">
      <div class="flex items-baseline gap-4">
        <span class="font-mono text-sm tabular-nums text-slate-400">05.</span>
        <h3 class="font-serif text-2xl font-bold text-grc-ink">Operational Resilience Snapshot</h3>
      </div>
      <span class="text-[11px] text-slate-400">DORA · APRA CPS 230 · MAS 658</span>
    </header>
        ...
        <tbody>
          {#each data.planRows.slice(0, 5) as r}
            ...
            <tr class="tr">
              ...
              <td class="td num text-xs">{r.plan.rtoMinutes >= 60 ? `${Math.round(r.plan.rtoMinutes / 60)}h` : `${r.plan.rtoMinutes}m`}</td>
              <td class="td num text-xs">{r.plan.rpoMinutes >= 60 ? `${Math.round(r.plan.rpoMinutes / 60)}h` : `${r.plan.rpoMinutes}m`}</td>
  ...
  <section class="mt-10 border-t border-slate-200 pt-8">
    <header class="mb-6 flex items-baseline justify-between">
      <div class="flex items-baseline gap-4">
        <span class="font-mono text-sm tabular-nums text-slate-400">06.</span>
        <h3 class="font-serif text-2xl font-bold text-grc-ink">Agent ROI Summary</h3>
      </div>
      <span class="text-[11px] text-slate-400">10 named agents · 30-day window</span>
    </header>
    <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
      <div class="rounded-lg bg-white p-4 ring-1 ring-inset ring-slate-200/70">
        ...
        <div class="mt-1 num text-[22px] font-semibold tracking-tight text-violet-700">
          S${(annualSavings / 1_000_000).toFixed(1)}<span class="ml-0.5 text-sm font-medium text-slate-400">M</span>
        </div>
      ...
      <div class="rounded-lg bg-white p-4 ring-1 ring-inset ring-slate-200/70">
        ...
        <div class="mt-1 num text-[22px] font-semibold tracking-tight text-violet-700">{fteSaved}<span class="ml-0.5 font-sans text-sm font-medium text-slate-400">×</span></div>
      ...
      <div class="rounded-lg bg-white p-4 ring-1 ring-inset ring-slate-200/70">
        ...
        <div class="mt-1 num text-[22px] font-semibold tracking-tight text-slate-800">
    ...
    <div class="mt-6 rounded-lg bg-white p-5 ring-1 ring-inset ring-slate-200/70">
    ...
    <p class="mt-5 text-[11px] leading-relaxed text-slate-500">
      ...
      Run-rate cost of approximately <span class="num">S${Math.round(fleetCostMonthly).toLocaleString()}</span> per month against an avoided
      cost of <span class="num">~S${Math.round((annualSavings / 12) / 1000).toLocaleString()}K</span> per month — a
      <span class="num font-semibold text-violet-700">{Math.round((annualSavings / 12) / Math.max(fleetCostMonthly, 1))}×</span> ROI.
    </p>
  </section>
  ...
  <div class="mt-10 flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-4 text-[11px] text-slate-400">
    ...
    <div>Confidential — Board of Directors only · NTT GRC Hub · v1.0.0</div>
  </div>
</div>
