<script lang="ts">
  ...
  $: radarSeries = [
    { name: 'Current', color: '#047857', values: top8.map((f) => f.score ?? 0) },
    { name: 'Target', color: '#a855f7', values: top8.map(() => 90) }
  ];
  ...
  $: roiBarSeries = (() => {
    ...
    return [
      { name: 'Cost (S$)', color: '#047857', data: cost },
      { name: 'FTE-hours saved', color: '#a855f7', data: fte }
    ];
  })();
...
<div class="mx-auto max-w-4xl">
  <!-- ============================================================ -->
  <!-- Magazine title strip                                          -->
  <!-- ============================================================ -->
  <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-700 via-emerald-600 to-emerald-800 px-8 py-6 text-white shadow-card-hover">
    <div class="absolute -right-12 -top-12 h-48 w-48 rounded-full bg-emerald-400/20 blur-3xl"></div>
    <div class="absolute -bottom-16 -left-16 h-56 w-56 rounded-full bg-emerald-900/40 blur-3xl"></div>
    <div class="relative flex flex-wrap items-end justify-between gap-4">
      <div class="min-w-0">
        <div class="mb-1 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-emerald-100">
          <span class="inline-flex h-1.5 w-6 rounded-full bg-emerald-300"></span>
          NTT GRC Hub · Board Risk Pack
        </div>
        <h2 class="font-serif text-3xl font-bold leading-tight">
          BOARD RISK PACK — {month.toUpperCase()}
        </h2>
        <p class="mt-1 text-sm text-emerald-100">{tenantName} · Confidential to the Board of Directors</p>
      </div>
      <div class="flex items-center gap-3">
        <AgentTypeBadge type="intelligent" />
        <div class="rounded-lg bg-white/10 px-3 py-2 text-right ring-1 ring-inset ring-white/20 backdrop-blur">
          <div class="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wider text-emerald-100">
            <Sparkles class="h-3 w-3" />
            Generated by Board Narrator
          </div>
          <div class="mt-0.5 text-[11px] text-emerald-50">7 min read · {fmtRel(generatedAt)}</div>
        </div>
      </div>
    </div>
  </div>
  ...
  <section class="card mt-8 p-8 lg:p-10">
    ...
    <div class="space-y-4 text-slate-700">
      {#each paragraphs as p, i}
        <p class="leading-relaxed {i === 0 ? 'text-lg font-medium text-slate-900 first-letter:float-left first-letter:mr-2 first-letter:font-serif first-letter:text-5xl first-letter:font-bold first-letter:leading-none first-letter:text-grc-primary' : 'text-[15px]'}">
          {p}
        </p>
      {/each}
    </div>
  ...
  <section class="card mt-8 p-8 lg:p-10">
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
      {#each data.topRisks.slice(0, 5) as r, i}
        ...
        {@const rank = residual(r)}
        <div class="rounded-xl border border-slate-200 bg-gradient-to-br from-white to-slate-50/40 p-5 {i === 0 ? 'md:col-span-2 ring-2 ring-emerald-500/30 bg-gradient-to-br from-emerald-50/80 to-white' : ''}">
  ...
  <section class="card mt-8 overflow-hidden p-0 lg:p-0">
    <div class="bg-gradient-to-br from-violet-50 via-white to-emerald-50/40 p-8 lg:p-10">
      <header class="mb-5 flex items-baseline justify-between border-b border-slate-200 pb-3">
        <div>
      ...
      <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <div class="rounded-xl border border-violet-200/80 bg-white p-4 shadow-card">
          ...
          <div class="mt-1 font-mono text-3xl font-bold text-violet-700">{fteSaved}</div>
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-card">
          ...
          <div class="mt-1 font-mono text-3xl font-bold text-slate-800">
