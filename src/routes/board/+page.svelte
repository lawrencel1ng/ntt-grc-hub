...
<div class="mx-auto max-w-4xl">
  ...
  <!-- ============================================================ -->
  <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-violet-700 via-violet-600 to-violet-800 px-8 py-6 text-white shadow-card-hover">
    <div class="absolute -right-12 -top-12 h-48 w-48 rounded-full bg-violet-400/20 blur-3xl"></div>
    <div class="absolute -bottom-16 -left-16 h-56 w-56 rounded-full bg-violet-900/40 blur-3xl"></div>
    <div class="relative flex flex-wrap items-end justify-between gap-4">
      <div class="min-w-0">
        <div class="mb-1 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-violet-100">
          <span class="inline-flex h-1.5 w-6 rounded-full bg-violet-300"></span>
          NTT GRC Hub · Board Risk Pack
        </div>
        <h2 class="font-serif text-3xl font-bold leading-tight">
          BOARD RISK PACK — {month.toUpperCase()}
        </h2>
        <p class="mt-1 text-sm text-violet-100">{tenantName} · Confidential to the Board of Directors</p>
      </div>
      <div class="flex items-center gap-3">
        ...
        <div class="rounded-lg bg-white/10 px-3 py-2 text-right ring-1 ring-inset ring-white/20 backdrop-blur">
          <div class="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wider text-violet-100">
            <Sparkles class="h-3 w-3" />
            Generated by Board Narrator
          </div>
          <div class="mt-0.5 text-[11px] text-violet-50">7 min read · {fmtRel(generatedAt)}</div>
        </div>
      </div>
    </div>
  </div>
  ...
  <section class="card mt-8 p-8 lg:p-10">
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
      {#each data.topRisks.slice(0, 5) as r, i}
        ...
        {@const rank = residual(r)}
        <div class="rounded-xl border border-slate-200 bg-gradient-to-br from-white to-slate-50/40 p-5 {i === 0 ? 'md:col-span-2 ring-2 ring-violet-500/30 bg-gradient-to-br from-violet-50/80 to-white' : ''}">
  ...
  <section class="card mt-8 overflow-hidden p-0 lg:p-0">
    <div class="bg-gradient-to-br from-violet-50 via-white to-fuchsia-50/40 p-8 lg:p-10">
      <header class="mb-5 flex items-baseline justify-between border-b border-slate-200 pb-3">
        <div>
